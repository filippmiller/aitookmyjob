...
async function storageUpsertIdentity(identity) {
  if (!usePostgres) {
    ...
    const next = { emailVerified: true, phoneVerified: false, phoneOtpAttempts: 0, ...rows[idx], ...identity, updatedAt: new Date().toISOString() };
...
async function storageInsertForumReply(reply) {
  if (!usePostgres) {
    ...
    if (idx >= 0) { topics[idx].replies = Number(topics[idx].replies || 0) + 1; topics[idx].lastUpdate = reply.createdAt; writeJsonArray(forumTopicsPath, topics); }
    return;
  }
...
async function storageDeleteUser(userId) {
  if (!usePostgres) {
    writeJsonArray(usersPath, readJsonArray(usersPath).filter((u) => u.id !== userId));
    writeJsonArray(authIdentitiesPath, readJsonArray(authIdentitiesPath).filter((x) => x.userId !== userId));
    writeJsonArray(telegramLinksPath, readJsonArray(telegramLinksPath).filter((x) => x.userId !== userId));
    writeJsonArray(storiesPath, readJsonArray(storiesPath).filter((s) => s.submittedBy !== userId));
  } else {
...
module.exports = {
  ...
  // File I/O
  readJsonArray, writeJsonArray, readStories, writeStories,
